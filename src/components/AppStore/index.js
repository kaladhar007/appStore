  ...
  render() {
    ...
    return (
      <div className="page">
        ...
        <ul className="ulButtonContainer">
          {tabsList.map(obj => (
            <TabItem
              objDetails={obj}
              isActive={obj.tabId === activeTabId}
              key={obj.tabId}
              onClickChangeActiveTabId={this.onClickChangeActiveTabId}
            />
          ))}
        </ul>
        <ul className="AppItemContainer">
          {filteredAppList.map(obj => (
            <AppItem objDetails={obj} key={obj.appId} />
          ))}
        </ul>
      </div>
    )
